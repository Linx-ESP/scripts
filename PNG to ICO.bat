...
SET directoryPath=C:\Programas\Scripts\Ico Converter\
...
IF EXIST %argPath%\* (
	...
	FOR %%f IN (%argPath%\*.png %argPath%\*.bmp %argPath%\*.gif %argPath%\*.jpg %argPath%\*.jpeg %argPath%\*.svg) DO (
		...
		"%directoryPath%ImageMagick\convert.exe" "%%f" -define icon:auto-resize=256,128,96,64,48,32,24,16 "%%~df%%~pf%%~nf.ico"
	)
